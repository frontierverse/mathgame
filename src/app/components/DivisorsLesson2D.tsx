  ...
  return (
    <section className="relative flex min-h-[520px] min-w-0 max-w-full flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface-raised)] shadow-[0_12px_30px_rgba(111,92,130,0.09)] sm:min-h-[620px] lg:min-h-0">
      <header className="relative z-20 flex items-center justify-between gap-3 px-4 pb-1 pt-4 sm:px-6 sm:pt-5">
        <h2 className="sr-only">{value}의 약수</h2>
        ...
        >
          <input
            ...
            aria-label="약수를 확인할 자연수"
            aria-describedby="divisor-input-range"
          ...
          <span
            aria-hidden="true"
            className="whitespace-nowrap text-2xl font-bold tracking-[-0.04em] sm:text-3xl"
          >
            의 약수
          </span>
          <span id="divisor-input-range" className="sr-only">
            1 이상 60 이하의 자연수를 입력하세요.
          </span>
          <button
            type="submit"
            disabled={nextValue === null}
            aria-label="입력한 수의 약수 보기"
      ...
      <div
        role="img"
        aria-label={`${value}의 블록이 약수쌍에 따라 직사각형으로 바뀝니다. ${value}의 약수는 ${divisors.join(", ")}입니다.`}
        className="relative min-h-[390px] flex-1 sm:min-h-[450px]"
      >
        <DivisorsConcept2D key={`${value}-${replayKey}`} value={value} />
      </div>
      <p className="sr-only" aria-live="polite">
        {value}의 약수는 {divisors.join(", ")}입니다.
      </p>
    </section>
  );
}
